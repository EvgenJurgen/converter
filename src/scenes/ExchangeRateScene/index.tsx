...
export const ExchangeRateScene = () => {
  ...
  const getSelectedCurrencyName = async () => {
    try {
      const selectedCurrencyName = await AsyncStorage.getItem(
        asyncStorageItemName
      );
      ...
      {selectedCurrency.name ? (
        <>
          {isLoading ? (
            ...
            <ScrollList convertedCurrencies={exchangeRate} />
          )}
        </>
      ) : (
        <Link
          to={{ screen: HOME_SCENE_NAME }}
          style={styles.noBaseCurrencyMessage}
        >
